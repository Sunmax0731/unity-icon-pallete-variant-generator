...
if (-not (Select-String -Path $logPath -Pattern "ISSUE48_RGB_SOURCE_ERASER_VALIDATION=PASS" -Quiet)) {
    throw "Unity RGB source eraser validation did not emit pass marker. Log: $logPath"
}

if (-not (Select-String -Path $logPath -Pattern "ISSUE48_JPG_SOURCE_ERASER_VALIDATION=PASS" -Quiet)) {
    throw "Unity JPG source eraser validation did not emit pass marker. Log: $logPath"
}

if (-not (Select-String -Path $logPath -Pattern "ISSUE48_JPG_INTERNAL_PNG_CONVERSION_VALIDATION=PASS" -Quiet)) {
    throw "Unity JPG internal PNG conversion validation did not emit pass marker. Log: $logPath"
}

if (-not (Select-String -Path $logPath -Pattern "ISSUE48_JPG_WINDOW_ERASER_VALIDATION=PASS" -Quiet)) {
    throw "Unity JPG window eraser validation did not emit pass marker. Log: $logPath"
}

if (-not (Select-String -Path $logPath -Pattern "ISSUE48_EXPORT_ALPHA_TRANSPARENCY_VALIDATION=PASS" -Quiet)) {
