...
GitHub Releases から `PaletteVariantGenerator_v1.0.3.zip` をダウンロードして展開し、パッケージフォルダを Unity プロジェクトに追加します。
...
- Preview更新は操作直後にキューへ積まれ、短い遅延後に最新の要求だけを処理します。連続操作中の不要な再計算を抑えます。
...
- 選択色情報パネルには、HEX、RGBA、所属グループ、ピクセル数、置換後色、色別ルール状態、透明置換の有無が表示されます。
